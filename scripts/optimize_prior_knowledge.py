...
def get_args(): 
    ...
    parser.add_argument("--node_channels", type=int, default=124,
...
def get_node_features(args, dgi_params={'epochs':100, 'dropout':0., 'layers':10, 'conv':'gin', 'lr':1e-4}, 
    ...
    if args.features == 'reactome': 
        ...
        model = VAE(x, dropout=0., hidden_channels=512, latent_dim=args.node_channels)
        ...
        N2V = Node2Vec(data.edge_index_dict['function','to','function'], embedding_dim=args.node_channels, walk_length=20, context_size=10, 
        ...
        dgi = DGI(data_,  args.node_channels, dropout=dgi_params['dropout'], layers=dgi_params['layers'], conv=dgi_params['conv'])
        ...
        print('training node2vec model...')
        N2V = Node2Vec(data.edge_index_dict['function','to','function'], embedding_dim=args.node_channels//3, walk_length=20, context_size=10, 
        ...
        data_.num_nodes = len(data['node_names_dict']['function'])
        data_.x = torch.eye(data_.num_nodes) # one hot encode nodes 
        dgi = DGI(data_, args.node_channels//3, dropout=dgi_params['dropout'], layers=dgi_params['layers'], conv=dgi_params['conv'])
        dgi.train(device, epochs=dgi_params['epochs'], lr=dgi_params['lr'])
        ...
        uni2rea = pd.read_csv(f'../../data/UniProt2Reactome_All_Levels.txt', sep='\t', header=None).rename({i:n for i,n in enumerate(['uniprot', 'pathway_id', 'url', 'name', '-', 'species'])}, axis=1)
        x, pathway_ids, pathway_names = get_function_pathway_features(data, uni2rea, K=1000)
        model = VAE(x, dropout=0., hidden_channels=512, latent_dim=args.node_channels//3)
        _,_ = model.train(device='cuda', epochs=1000, patience=-1, lr=1e-3, beta=1, verbose=True)
        z  = model.embed(device='cpu')
...
if __name__ == '__main__': 
    ...
    scaler = RewardScaler(alpha=args.alpha, clip=3, warmup=args.warmup)
    ...
    for iter in range(args.outer_iters): 
        ...
        if iter >= args.warmup: 
            if first: 
                print()
                print('warmup complete; starting gradient updates...'); first=False
            if args.alg == 'reinforce': 
                loss = -reward_scaled*m.log_prob(action).sum() + args.l1_penalty*logits.mean() + args.entropy*m.entropy().mean()
                loss.backward()
                optim.step() 
            elif args.alg == 'ppo': 
                ppo.update_actor(args, actor, x, reward_scaled, logits, action, optim, 
                                 iter=iter, iters=args.ppo_iters, clip_param=args.clip_param, target_kl=args.target_kl)
            else: 
                raise Exception('unrecognized `alg` argument')
