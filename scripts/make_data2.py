...
from gsnn.proc.utils import get_x_drug_conc
...
def build_nx(func_df, targets, lincs): 
    ...
    for linc in lincs: 
        if ('RNA__' + linc) in G: 
            ...
            print(f'warning: {linc} does not have a respective RNA node. This will result in no predictive ability.')

    return G


def bfs_distance(G, start_node, depth):
    """
    ...
    :return: A dictionary mapping each node to its shortest path length from start_node
    """
...
def get_all_possible_paths_set(G, root, leaf, depth):
    """
    ...
    root_distances = bfs_distance(G, root, depth=depth)

    if leaf not in root_distances: 
        return set()
    
    rG = G.reverse() 
    # Step 2: Perform reverse BFS from the leaf
    leaf_distances = bfs_distance(rG, leaf, depth=depth)
    
    # Step 3: Combine the distances
    node_set = set()
    for node in G.nodes(): 
        if (node in root_distances) and (node in leaf_distances): 
            shortest_path_length = root_distances[node] + leaf_distances[node]
...
def filter_func_nodes(args, func_names, func_df, targets, lincs, drugs): 
    ...
    # for every drug
    #   get all paths to every LINCS/output
    #   filter all paths of length > filter_depth 
    # aggregate all paths
    # any drugs that do not appear - filter 
    # any func nodes that do not appear - filter 
    # any output nodes that do not appear - filter 

    nodes = set()
    for i,drug in enumerate(drugs): 
        for j,linc in enumerate(lincs): 
            print(f'filter progress: {i+1}/{len(drugs)} [{j+1}/{len(lincs)}]', end='\r')
            nodes = nodes.union( get_all_possible_paths_set(G, root='DRUG__' + drug, leaf = 'LINCS__' + linc, depth=args.filter_depth) )
...
def downstream_nodes(G, node, N, visited=None):
    ...
    # If we've reached the maximum depth, return the visited nodes so far
    if N == 0:
        return set()
    ...
    for n in G.successors(node):
        if n not in visited:
            ...
            downstream.add(n)
            downstream.update(downstream_nodes(G, n, N-1, visited))

    return downstream

def upstream_nodes(G, node, N, visited=None):
    # If visited is None, initialize it as an empty set
    if visited is None:
        visited = set()
    ...
    for n in G.predecessors(node):
        if n not in visited:
            # Recursively find downstream nodes
            upstream.add(n)
            upstream.update(upstream_nodes(G, n, N-1, visited))

    return upstream

def filter_func_nodes(args, func_names, func_df, targets, lincs, drugs): 
    # NOTE: this could be modifier to check for any upstream input rather than just drug (e.g., drug + omic)
    print('filtering function nodes...')
    G = build_nx(func_df, targets, lincs)

    # check that all DRUGS access at least one output within depth
    filter_drugs_mask = []
    nfilt=0
    for i,drug in enumerate(drugs):
        print(f'drug filter progress: {i}/{len(drugs)}', end='\r')
        dstream = downstream_nodes(G, 'DRUG__' + drug, args.filter_depth)
        n_lincs_ds = len([x for x in dstream if 'LINCS' in x])
        if n_lincs_ds == 0: 
            filter_drugs_mask.append(False)
            nfilt+=1
        else: 
            filter_drugs_mask.append(True)
    drugs = np.array(drugs)[np.array(filter_drugs_mask)].tolist()
    targets = targets[lambda x: x.pert_id.isin(drugs)]
    print(f'# of drugs filtered (no downsream LINCS) [depth={args.filter_depth}]: {nfilt}/{len(filter_drugs_mask)}')

    # rebuild graph with new drugs 
    G = build_nx(func_df, targets, lincs)

    # TODO: check that LINCS have at least one upstream drug 

    # Filter function nodes 
    filter_function_nodes_mask = [] # True if keep
    niso = 0
    nfilt = 0
    for i,fname in enumerate(func_names): 
        ...
        if fname not in G: 
            # isolates 
            filter_function_nodes_mask.append(False)
            niso+=1
            continue

        print(f'function filter progress: {i}/{len(func_names)}', end='\r')
        dstream = downstream_nodes(G, fname, args.filter_depth)
        ustream = upstream_nodes(G, fname, args.filter_depth)

        n_lincs_ds = len([x for x in dstream if 'LINCS' in x])
        n_drugs_us = len([x for x in ustream if 'DRUG' in x])

        if (n_lincs_ds == 0) or (n_drugs_us == 0): 
            filter_function_nodes_mask.append(False)
            nfilt += 1
        else: 
            filter_function_nodes_mask.append(True)

    print('# isolates removed:', niso)
    print(f'# of drugs filtered (no downsream LINCS) [depth={args.filter_depth}]: {nfilt}/{len(filter_function_nodes_mask)}')

    func_names = np.array(func_names)[np.array(filter_function_nodes_mask)].tolist()

    func_df = func_df[lambda x: x.source.isin(func_names) & (x.target.isin(func_names))]
    targets = targets[lambda x: x.target_name.isin(func_names)]

    # Filter DRUGS? any drugs that don't have an output downstream 
    # Filter LINCS? any lincs that don't have a drug upstream

    return func_names, func_df, targets, drugs
